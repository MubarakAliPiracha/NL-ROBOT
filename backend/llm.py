...
- Layout: keep about 1.5 m clear around the origin so the robot isn't spawned inside something, unless asked otherwise. Keep passages at least the robot's footprint width plus 0.8 m. Prefer 5-12 objects for "obstacle course/room/map", more only for mazes. Give objects short descriptive names. Use varied colours.
- Maps: build them like a designer. Mazes: lay out a grid of cells (cell size = robot footprint width + 1.0 m, at least 2 m), an outer boundary, interior walls that leave exactly one route (or a few) from the start cell to an exit, and merge collinear walls into long boxes. Rooms: outer walls with doorways, then furniture (tables, shelves, crates). Courts and arenas: boundary lines, hoops or goals from several pieces, sensible sizes. Make it look intentional: consistent wall height (about 1 m), coherent colours, no overlapping shapes, everything inside a sensible area in front of the robot (x from 1 to about 25, y within +-12).
...
def _call_anthropic(state: Dict, text: str, effort: Optional[str] = None) -> Dict:
    ...
    client = anthropic.Anthropic(timeout=90.0, default_headers=headers or None)
    kwargs: Dict = dict(
